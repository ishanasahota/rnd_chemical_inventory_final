...
try:
    ...
    # Use tuple (name, manufacturer) as key to group by both
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        manufacturer = str(row[5] or "").strip()  # Column F: Manufacturer (index 5)
        
        if any(keyword in name for keyword in keywords):
            try:
                # Use tuple of (name, manufacturer) as key
                key = (name, manufacturer)
                critical_summary[key] += float(remaining_qty)
            except (TypeError, ValueError):
                print(f"⚠Skipped '{name}' from '{manufacturer}' due to invalid Remaining Quantity: '{remaining_qty}'")
                continue
    ...
    msg_critical.text(
        ...
        "Combined remaining amounts by fluid name and manufacturer:\n\n"
